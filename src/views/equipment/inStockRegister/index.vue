...
export default {
  ...
  data() {
    return {
      ...
      dataList: {},
      fileUrl: `${process.env.VUE_APP_API_URL}/equipment/import`,
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
    onTemplateDown() {
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/download_TemplateFile?infoType=t_equipment`)
    },
     getAttachFile(query) {
    ...
    onUpload() {
      this.fileUrl = `${process.env.VUE_APP_API_URL}/equipment/import`,
