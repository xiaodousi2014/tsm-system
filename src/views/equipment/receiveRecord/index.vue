...
export default {
  ...
  methods: {
     ...
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_equipment_use" , file: query.file})
      .then((res) => {
    ...
    onExport() {
      ...
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/export?ids=${query.toString()}&&infoType=t_equipment_use`)
    },
    getAllField() {
      Http.getReciveTitle()
        .then((res) => {
