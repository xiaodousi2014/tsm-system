...
export default {
  ...
  methods: {
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_maps_check" , file: query.file})
      .then((res) => {
    ...
    onUploadFile() {
      ...
      this.fileUrl = `${process.env.VUE_APP_API_URL}/common/attachment/import?infoType=t_maps_check&id=${this.multipleSelection[0].id}`;
    ...
    onExport() {
      ...
      window.open(`${process.env.VUE_APP_API_URL}/maps/check/export?id=${this.multipleSelection[0].id}`)
