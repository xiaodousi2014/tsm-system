...
export default {
  ...
  data() {
    return {
      ...
      fileUrl: `${process.env.VUE_APP_API_URL}/maps/import`,
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
    onTemplateDown() {
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/download_TemplateFile?infoType=t_maps`)
    },
     getAttachFile(query) {
    ...
    onUpload() {
      this.fileUrl = `${process.env.VUE_APP_API_URL}/maps/import`,
