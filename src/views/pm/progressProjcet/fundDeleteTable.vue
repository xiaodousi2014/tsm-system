<template>
  <div>
    <div class="content_box my-el-table">
      <div class="menu-type">
        <span style="font-size: 20px; font-weight: 900">经费报销删除记录</span>
...
export default {
  ...
  methods: {
    getInfoType(){
      // 获取所有字段
      Api.getInfoTypeFunDelete().then((res) => {
        if (res.code == "0000") {
    ...
    getTableList() {
      ...
      Api.queryFundDellist(Utils.filterParams(params)).then((res)=>{
        if(res.code === '0000'){
          this.tableData = [];
          // this.total = 0;
    ...
    delectList() { // 撤销删除
      ...
      Api.dirtyFunDel(params).then((res)=>{
        if (res.code == '0000') {
