...
<!--      <custom-upload-file-form-put infoType="t_project_reimbursemen" :id="multipleSelectionInfo.id" :url="fileUrl" @close="close" />-->
<!--    </el-dialog>-->
...
export default {
  ...
  methods: {
    ...
    getTableList() {
      ...
      Api.querylist(Utils.filterParams(params)).then((res)=>{
        if(res.code === '0000'){
          this.tableData = [];
          // this.total = 0;
    ...
    delectList() { // 删除
      ...
      Api.delReimbursement(params).then((res)=>{
        if (res.code == '0000') {
    ...
    listCreate(event) { // 创建保存
      ...
      Api.addReimbursement(event).then((res)=>{
        if (res.code == '0000') {
    ...
    listEdit(event) { // 修改保存
      event.project_id = this.project_id
      event.project_name = this.project_name
      Api.putReimbursement(event).then((res)=>{
        if (res.code == '0000') {
          this.$message.success('操作成功！')
          this.close()
        }
    ...
    goDelectPage() { // 删除记录页面
      ...
      this.$router.push({path: "/pm/progress-projcet-fund-delete",
        query: {
    ...
    onTemplateDown() {
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/download_TemplateFile?infoType=t_docs`)
    },
    close() {
