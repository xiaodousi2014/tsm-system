...
export default {
  ...
  methods: {
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_material_check" , file: query.file})
      .then((res) => {
    ...
    onUploadFile() {
      ...
      this.fileUrl = `${process.env.VUE_APP_API_URL}/common/attachment/import?infoType=t_material_check&id=${this.multipleSelection[0].id}`;
    ...
    onExport() {
      ...
      window.open(`${process.env.VUE_APP_API_URL}/material/check/export?id=${this.multipleSelection[0].id}`)
