    ...
    >
      ...
      >
        ...
        >
          <el-form-item
            :label="item.comment"
            v-if="item.editable && (item.type == 'string' || item.type == 'date' || item.type == 'datetime' || item.type == 'int' || item.type == 'map' || item.type == 'list')"
          >
            <el-input
...
<script>
import Api from "@/api/system/right";
...
export default {
  ...
  methods: {
    ...
    onSumit() {
      // 处理已选中的权限数据
      this.form.code = JSON.stringify(this.rightCode);
