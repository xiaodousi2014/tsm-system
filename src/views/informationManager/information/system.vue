...
export default {
  ...
  data() {
    return {
      ...
      fileUrl: "http://139.198.188.175:8190/information/system/import",
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
   onTemplateDown() {
      window.open('http://139.198.188.175:8190/common/attachment/download_TemplateFile?infoType=t_information_system')
    },
     getAttachFile(query) {
    ...
    onRevoke() {
      ...
      Http.onThreeStockRevoke(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
    ...
    onUpload() {
      (this.fileUrl = "http://139.198.188.175:8190/information/system/import"),
    ...
    getTableList() {
      Http.getThreeStockList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
