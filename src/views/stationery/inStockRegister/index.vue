...
export default {
  ...
  data() {
    return {
      ...
      fileUrl: `${window.upLoadUrl}/stationery/import`,
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
     onTemplateDown() {
      window.open(`${window.upLoadUrl}/common/attachment/download_TemplateFile?infoType=t_stationery`)
    },
    getAttachFile(query) {
   ...
   // 上传附件
    ...
    onUpload() {
      (this.fileUrl = "${window.upLoadUrl}/stationery/import"),
