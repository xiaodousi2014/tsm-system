...
<script>
// 获取学生接口
import Api from "@/api/baseData/staff";
...
export default {
  ...
  methods: {
    ...
    getTableList() {
      ...
      Api.queryStaff(Utils.filterParams(params)).then((res) => {
        if (res.code === "0000") {
          this.tableData = [];
          // this.total = 0;
          console.log("读取循环的数据");
    ...
    edit() {
      ...
      if (this.multipleSelection.length > 1) {
        this.$message.warning("您一次只能修改一位业务参谋");
        return;
      }
    ...
    delectList() {
      ...
      if (this.multipleSelection.length > 1) {
        this.$message.warning("您一次只能删除一位业务参谋");
        return;
      }
