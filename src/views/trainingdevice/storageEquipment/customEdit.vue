...
debugger
        if (this.form.consume_years) {
            consume_years = JSON.parse(this.form.consume_years)

            this.jsonData = []

            Object.keys(consume_years).map((key) => {
                this.jsonData.push({
                    key: key,
                    value: consume_years[key]
                })
            })
        }
    },
    methods: {
        ...
        onSumit() {
            ...
            this.form.consume_years = JSON.stringify(jsonResult)
            this.$emit('listEdit')
        },
    },
}
