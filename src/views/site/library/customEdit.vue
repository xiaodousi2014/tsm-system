...
export default {
    ...
    data() {
        return {
            ...
            jsonData: [
                {
                    key: '',
                    value: '',fileTypeAddNameFun
                },
            ],
        }
    },
    mounted() {
        let maintance_fund_years = {
            key: '',
            value: '',
        }

        if (this.form.maintance_fund_years) {
            maintance_fund_years = JSON.parse(this.form.maintance_fund_years)

            this.jsonData = []

            Object.keys(maintance_fund_years).map((key) => {
                this.jsonData.push({
                    key: key,
                    value: maintance_fund_years[key]
                })
            })
        }
    },
    methods: {
        ...
        onSumit() {
            ...
            this.form.maintance_fund_years = JSON.stringify(jsonResult)
            this.$emit('listEdit')
        },
    },
}
