...
export default {
  ...
  data() {
    return {
      ...
      fileUrl: `${process.env.VUE_APP_API_URL}/information/import`,
  ...
  methods: {
     ...
     this.failMsg = msg;
      ...
      onTemplateDown() {
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/download_TemplateFile?infoType=t_information`)
    },
     getAttachFile(query) {
   ...
   // 上传附件
    ...
    onRevoke() {
      ...
      Http.onOneStockRevoke(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
    ...
    onUpload() {
      this.fileUrl = `${process.env.VUE_APP_API_URL}/information/import`,
    ...
    getTableList() {
      Http.getOneStockList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
