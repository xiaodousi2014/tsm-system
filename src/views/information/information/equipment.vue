...
export default {
  ...
  data() {
    return {
        ...
        indexArray: [
          {
            ...
            col_name: "data_table",
            indexType: "1",
            value: "t_information_electronic",
          },
        ],
        pageNum: 1,
        pageCount: 10,
      ...
      fileUrl: `${process.env.VUE_APP_API_URL}/information/electronic/import`,
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
    onTemplateDown() {
      window.open(`${process.env.VUE_APP_API_URL}/common/attachment/download_TemplateFile?infoType=t_information_electronic`)
    },
     getAttachFile(query) {
   ...
   // 上传附件
    onUploadFile() {
      ...
      this.fileUrl = `${process.env.VUE_APP_API_URL}/common/import/attachment/upload?import_id=${this.dataId}`;
    ...
    onRevoke() {
      ...
      Http.onTwoStockRevoke(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
    ...
    onUpload() {
      this.fileUrl = `${process.env.VUE_APP_API_URL}/information/electronic/import`,
      this.exportModal = true;
    },
    getAllField() {
      Http.getTwoStockTitle()
        .then((res) => {
          if (res.code == "0000") {
    ...
    getTableList() {
      Http.getTwoStockList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
