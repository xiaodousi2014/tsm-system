...
export default {
  ...
  methods: {
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_information_electronic" , file: query.file})
      .then((res) => {
    ...
    listEdit() {
      Http.onTwoListEdit(this.multipleSelectionInfo)
        .then((res) => {
          if (res.code == "0000") {
    ...
    // 撤销操作
    onRevoke() {
      this.$router.push("delete-list-two");
    },
    getAllField() {
      Http.getTwoStorageTitle()
        .then((res) => {
          if (res.code == "0000") {
    ...
    getTableList() {
      Http.getTwoStorageList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
    ...
    deleteSure() {
      Http.onTwoStorageDelete(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
