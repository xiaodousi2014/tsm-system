...
export default {
  ...
  methods: {
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_information_system" , file: query.file})
      .then((res) => {
    ...
    listEdit() {
      Http.onThreeListEdit(this.multipleSelectionInfo)
        .then((res) => {
          if (res.code == "0000") {
    ...
    // 撤销操作
    onRevoke() {
      this.$router.push("delete-list-three");
    },
    getAllField() {
      Http.getThreeStorageTitle()
        .then((res) => {
          if (res.code == "0000") {
    ...
    getTableList() {
      Http.getThreeStorageList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
    ...
    deleteSure() {
      Http.onThreeStorageDelete(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
