...
export default {
  ...
  data() {
    return {
      ...
      infoType: "t_information_electronic_dirty",
    };
  },
  mounted() {
    this.getAllField();
  ...
  methods: {
     ...
     getAttachFile(query) {
      ...
      Http.getAttachFile({id:query.row.id, infoType: "t_information_electronic_dirty" , file: query.file})
      .then((res) => {
    ...
    getTableList() {
      Http.getTwoDeleteStorageList(this.query)
        .then((res) => {
          if (res.code == "0000") {
            this.tableData = [];
            this.total = 0;
    ...
    // 撤销操作
    onRevoke() {
      Http.putTwoStorageList(this.multipleSelection)
        .then((res) => {
          if (res.code == "0000") {
