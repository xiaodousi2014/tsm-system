  ...
  <div class="content_box">
    <el-row>
      <el-col :span="10">
        ...
        <div
          v-for="field in field_data"
          :key="field.name"
          :class="[
            'field_box',
            fieldInfo.comment === field.comment && 'field_box_active',
          ]"
          @click="edit(field)"
        >
          {{ field.comment }}
      ...
      <el-col :span="11" :offset="1">
        <el-divider content-position="left"
          >属性 - {{ fieldInfo.comment }}</el-divider
        >
        <el-input
          size="small"
          v-model="value"
          style="width: 120px; margin-bottom: 8px"
          :disabled="!fieldInfo || !fieldInfo.name"
        />
        <el-button size="small" @click="add">添加</el-button><br />
        <div
          v-for="(field, index) in fieldInfo.itemdata"
          :key="field"
          class="field_box"
        >
          {{ field }}
        ...
        <el-button
          ...
          size="small"
          @click="submit()"
          :disabled="!fieldInfo.itemdata || fieldInfo.itemdata.length == 0"
          >提交</el-button
        >
      </el-col>
    </el-row>
  ...
  methods: {
    ...
    edit(field) {
      ...
      if (!Array.isArray(field.itemdata)) {
        itemdata =
          field.itemdata && field.itemdata.length > 0
            ? field.itemdata.split(",")
            : [];
      } else {
